...
var confeccionMujer = require('../models/confeccion-mujer');
var controller = {
 save: (req, res) => {
    ...
    var params = req.body;
        ...
        var confeccion = new confeccionMujer();
        //asignar valores
        confeccion.busto = params.busto;
        confeccion.bajoBusto = params.bajoBusto;
        confeccion.manga = params.manga;
        confeccion.hombro = params.hombro;
...
getMedidas: (req, res) => {
    //Find 
    confeccionMujer.find({}).exec((err, medidas) => {
        if (err) {
...
getConfeccionEspecifico: (req, res) => {
    ...
    //Buscar el producto
    confeccionMujer.findById(confeccionId, (err, confeccion) => {
        if (err || !confeccion) {
            return res.status(404).send({
                status: 'error',
                message: 'No existe el producto'
...
updateConfeccion: (req, res) => {
    ...
    //Validar los datos
        ...
        //Find and update
        confeccionMujer.findOneAndUpdate({ _id: confeccionId }, params, { new: true }, (err, medidasUpdate) => {
            if (err) {
                return res.status(500).send({
                    status: 'error',
...
uploadMedidas: (req, res) => {
    ...
    if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg' && file_ext != 'gif') {
        ...
        if(confeccionId){
            ...
            confeccionMujer.findOneAndUpdate({ _id: confeccionId }, { image: file_name }, { new: true }, (err, medidaUpdated) => {
...
getImageMujer: (req, res) => {
    var file = req.params.image;
    var path_file = './upload/medidasMujer/' + file;
